...
def get_mmsk(lam, miu, s, k, cw=None, cs=None, n=None):
    ...
    if(n):
        if(n <= s):
            ...
            pn = (math.pow(lam / miu, n) /
                  (math.factorial(s) * math.pow(s, n-s))) * p0
        else:
            pn = 0

    lq_1 = (p0 * math.pow(lam / miu, s) * rho) / \
        (math.factorial(s) * math.pow(1 - rho, 2))
    lq_2 = 1 - math.pow(rho, k - s) - ((k - s) *
                                       math.pow(rho, k - s) * (1 - rho))
    lq = lq_1 * lq_2
